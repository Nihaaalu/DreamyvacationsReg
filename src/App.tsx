...
export default function App() {
  ...
  const validateForm = (): boolean => {
    ...
    const emptyRoom = formData.rooms.length === 0 || formData.rooms.some(r => !r.trim());
    if (emptyRoom) {
      tempErrors.rooms = 'Please assign at least one room.';
    }
  ...
  const toggleRoomSelection = (roomNum: string) => {
    const currentRooms = formData.rooms.filter(r => r.trim() !== '');
    let updated: string[];
    if (currentRooms.includes(roomNum)) {
      updated = currentRooms.filter(r => r !== roomNum);
      showNotification(`Room ${roomNum} unassigned.`, 'warning');
    } else {
      updated = [...currentRooms, roomNum];
      showNotification(`Room ${roomNum} assigned successfully.`, 'success');
    }
    updateField('rooms', updated);
  };
          ...
          <div className="bg-[#050505] border border-zinc-800 rounded-2xl p-5 lg:p-6 space-y-4">
            ...
            <div className="space-y-4">
              <div className="space-y-1.5">
                ...
                <div className="w-full overflow-hidden bg-[#020202] p-3 rounded-lg border border-zinc-900 shadow-inner min-h-[50px] flex items-center">
                  <div className="flex flex-wrap items-center gap-2 w-full">
                    {formData.rooms.filter(r => r.trim() !== '').length > 0 ? (
                      formData.rooms.filter(r => r.trim() !== '').map((room, index) => (
                        <div
                          key={index}
                          onClick={() => toggleRoomSelection(room)}
                          className="flex items-center gap-1.5 bg-[#09090b]/80 hover:bg-[#121215]/80 rounded border border-zinc-800 px-3 py-1.5 text-white font-mono font-bold text-xs select-none cursor-pointer group transition-colors duration-150"
                          title="Click to remove room"
                        >
                          <span>{room}</span>
                          <span className="text-zinc-600 group-hover:text-rose-400 transition-colors duration-150 font-bold ml-1">×</span>
                        </div>
                      ))
                    ) : (
                      <span className="text-zinc-600 text-[11px] uppercase tracking-wider font-semibold italic">No rooms assigned</span>
                    )}
                  </div>
                </div>
                {errors.rooms && (
              ...
              {/* Room Buttons Selector Grid */}
              <div className="space-y-2.5 pt-2 border-t border-zinc-900/60">
                <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-1">
                  <span className="text-xs font-bold text-white uppercase tracking-wider">
                    Selected Rooms: {formData.rooms.filter(r => r.trim() !== '').length}
                  </span>
                  <span className="text-[10px] text-zinc-500 font-medium uppercase tracking-wider">
                    Click room numbers to assign rooms
                  </span>
                </div>
                ...
                <div className="space-y-2.5">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {['101', '102', '103', '104', '105', '106', '107', '108'].map((roomNum) => {
                      const isSelected = formData.rooms.includes(roomNum);
                      return (
                        <button
                          key={roomNum}
                          type="button"
                          onClick={() => toggleRoomSelection(roomNum)}
                          className={`min-w-[50px] h-9 text-xs font-mono font-bold rounded-lg border transition-all duration-150 cursor-pointer ${
                            isSelected
                  ...
                  {/* Row 2: Rooms 201-205 */}
                  <div className="flex flex-wrap gap-1.5">
                    {['201', '202', '203', '204', '205'].map((roomNum) => {
                      const isSelected = formData.rooms.includes(roomNum);
                      return (
                        <button
                          key={roomNum}
